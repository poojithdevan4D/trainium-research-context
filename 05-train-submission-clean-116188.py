"""Trainium Frontier Phase 1 submission: dense GPT, 6 layers x 1024 wide, trained from scratch.

Model: untied token embedding and head, RMSNorm, rotary embeddings with QK-norm (fused NKI
kernel), ReLU^2 MLP, value embeddings on alternating layers, learned residual/input mix, causal
two-tap depthwise convolution (NKI kernel), softcap-15 cross-entropy (NKI kernel), BF16 branch
inputs with FP32 residuals and statistics. Optimizer: Muon (Newton-Schulz, compiled body) for the
matrices, AdamW for embeddings/head/scalars, wall-clock LR and weight-decay schedules. Training
runs on four LNC2 ranks with data parallelism and optimizer-state ownership. `load_for_eval`
returns the trained model for the organiser's scorer.
"""
...
class MLP(nn.Module):
    ...
    def forward(self, x: torch.Tensor, fc: torch.Tensor | None=None) -> torch.Tensor:
        ...
        result = _FusedMLP.apply(x.reshape(tokens, x.shape[-1]), up, down)
...
class Block(nn.Module):
    ...
    def forward(self, x: torch.Tensor, x0: torch.Tensor, ve: torch.Tensor | None, cos: torch.Tensor, sin: torch.Tensor) -> torch.Tensor:
        ...
        attn_out = self.attn(norm(q_in), norm(k_in), norm(v_in), ve, cos, sin)
        x = x + attn_out
        x = x + self.mlp(norm(x))
        return x
    ...
    def setup_optimizer(self):
        ...
        for _i, _block in enumerate(self.transformer.h):
            for _p in _block.parameters():
                depth_of[id(_p)] = _i
        for shape, transposed in sorted({(p.shape, False) for p in matrix_params}, key=lambda st: (st[0], st[1])):
            params = [p for p in matrix_params if p.shape == shape and False == transposed]
...
def build_config() -> GPTConfig:
    """Model configuration from the module constants. N_EMBD > 0 sets the width directly; otherwise the
    width is DEPTH * ASPECT_RATIO rounded up to a multiple of HEAD_DIM.
    """
    depth = None
...
def load_for_eval(checkpoint_path: str, device: torch.device) -> nn.Module:
    """Return the trained model in eval mode. The rotary tables are not parameters and are sized for
...
def main() -> None:
    global PACK_FACTOR
    global loss_scale_clean, loss_scale_skips
    global N_EMBD, SEQ_LEN
    ...
    parser.add_argument('--compile', action=argparse.BooleanOptionalAction, default=os.environ.get('NEURON_COMPETITION_R1_COMPILE', '1') != '0', help="torch.compile(backend='neuron') for the model forward/backward. Default on.")
    parser.add_argument('--compile-sdpa-direct', action=argparse.BooleanOptionalAction, default=os.environ.get('NEURON_COMPETITION_R1_COMPILE_SDPA_DIRECT', '1') != '0', help='Call F.scaled_dot_product_attention directly inside the compiled graph. Default on.')
    parser.add_argument('--nki-local-conv', action=argparse.BooleanOptionalAction, default=os.environ.get('NEURON_COMPETITION_R1_NKI_LOCAL_CONV', '1') != '0', help='Use the NKI two-tap causal convolution kernel on Neuron when the shapes allow it. Default on.')
    parser.add_argument('--bf16-norm-output', action=argparse.BooleanOptionalAction, default=os.environ.get('NEURON_COMPETITION_R1_BF16_NORM_OUTPUT', '1') != '0', help='Keep FP32 RMS statistics and residuals but feed the branches BF16 normalized inputs. Default on.')
    parser.add_argument('--ngram-ve', action=argparse.BooleanOptionalAction, default=os.environ.get('NEURON_COMPETITION_R1_NGRAM_VE', '0') != '0', help='Not part of this build; must be passed as --no-ngram-ve.')
    ...
    parser.add_argument('--qk-shift', action=argparse.BooleanOptionalAction, default=os.environ.get('NEURON_COMPETITION_R1_QK_SHIFT', '0') != '0', help='Not part of this build; must be passed as --no-qk-shift.')
    parser.add_argument('--seed', type=int, default=int(os.environ.get('NEURON_COMPETITION_R1_SEED', '42')), help='torch.manual_seed value. Default 42.')
    _hp('--seq-len', SEQ_LEN, int, 'Training context length in tokens.')
    _hp('--n-embd', N_EMBD, int, 'Model width (must be a multiple of HEAD_DIM); 0 derives it from DEPTH * ASPECT_RATIO.')
    parser.add_argument('--clip-after-reduce', action=argparse.BooleanOptionalAction, default=os.environ.get('NEURON_COMPETITION_R1_CLIP_AFTER_REDUCE', '1') != '0', help='Average gradients across ranks before clipping. Default on.')
    _hp('--pack-factor', PACK_FACTOR, int, 'Rows per micro-batch: the loader fetches PACK_FACTOR*SEQ_LEN tokens per row and reshapes them to PACK_FACTOR rows of SEQ_LEN; grad_accum_steps falls as this rises (TOTAL_BATCH_SIZE is fixed).')
    args = parser.parse_args()
    if args.clip_after_reduce != True:
        parser.error('this submission fixes CLIP_AFTER_REDUCE=True; the launch flag must agree')
    loss_scale_clean = 0
    loss_scale_skips = 0
    PACK_FACTOR = args.pack_factor
    SEQ_LEN = args.seq_len
    N_EMBD = args.n_embd
    if args.nki_local_conv != True:
        parser.error('this submission fixes USE_NKI_LOCAL_CONV=True; the launch flag must agree')
    if args.ngram_ve != False:
        parser.error('this submission fixes USE_NGRAM_VE=False; the launch flag must agree')
    if not 0.0 <= 1.0 <= 1.0:
        parser.error(f'--byte-wte-init-mix must be in [0, 1], got {1.0}')
    if args.qk_shift != False:
        parser.error('this submission fixes USE_QK_SHIFT=False; the launch flag must agree')
    if not 0.0 < (0.35, 0.75)[0] <= (0.35, 0.75)[1] < 1.0:
        raise ValueError(f'--batch-ramp-bounds needs 0 < A->B <= B->C < 1, got {(0.35, 0.75)}')
    if not all((0.0 < d < 1.0 for d in (0.995,))):
        raise ValueError(f'--weight-ema-decay needs one or more values in (0,1), got {(0.995,)}')
    if not 0.0 <= 0.75 < 1.0:
        raise ValueError(f'--weight-ema-start-frac must be in [0,1), got {0.75}')
    if args.compile_sdpa_direct != True:
        parser.error('this submission fixes COMPILE_SDPA_DIRECT=True; the launch flag must agree')
    if args.bf16_norm_output != True:
        parser.error('this submission fixes USE_BF16_NORM_OUTPUT=True; the launch flag must agree')
    ...
    print0(f'gradient_clip={GRAD_CLIP:g} clip_after_reduce={True}')
    ...
    while step < args.num_steps:
        ...
        for micro_index in range(grad_accum_now):
            ...
            (loss / grad_accum_steps * LOSS_SCALE).backward()
            x, y, loader_state = fetch()
        ...
        for group in optimizer.param_groups:
            group['lr'] = group['initial_lr'] * 1.0 * (lrm_flat if group.get('no_lr_decay') else lrm)
            if group['kind'] == 'muon':
...
class OwnedOptimizer:
    ...
    def __init__(self, optimizer):
        ...
        self.pad_singleton_muon = bool(True)
