...
def _read_row_group_batches(path, rg_idx, tokenizer_batch_size, epoch):
...
def main() -> None:
    ...
    parser.add_argument('--ngram-ve-freeze-epoch', type=int, default=NGRAM_VE_FREEZE_EPOCH, help='IMPL-2: from the first step whose batch comes from loader epoch >= N, stop updating the n-gram tables (their gradients are dropped before the all-reduce; lookups continue). 0 (default) = off, bit-identical. The compiled graph is unchanged, so there is no recompilation.')
    parser.add_argument('--fresh-tail-frac', type=float, default=FRESH_TAIL_FRAC, help='IMPL-3: walk each rank\'s row groups as A, A, B where B is the last FRAC of them, so the end of a ~1.7-epoch run trains on unseen documents. 0 (default) = off (organizer loader, bit-identical). Uses the in-file requeue loader; compare against --requeue-identity.')
